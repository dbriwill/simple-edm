...
ARCHETYPES_LOCATION=../plugins/archetypes
...
current_version=$(grep '<ged.archetypes.version>' pom.xml | sed 's/^.*>\(.*\)<.*$/\1/')
...
sed -i -e "s@<ged.archetypes.version>${current_version}</ged.archetypes.version>@<ged.archetypes.version>${release_version}</ged.archetypes.version>@" pom.xml
...
echo "Updating version in pom.xml"
sed -i -e "s@<ged.archetypes.version>${release_version}</ged.archetypes.version>@<ged.archetypes.version>${next_snapshot_version}-SNAPSHOT</ged.archetypes.version>@" pom.xml
